...
本实验针对物联网传感器数据，设计并实现了一个基于机器学习的异常检测系统。实验采用Isolation Forest（孤立森林）算法\cite{Liu08}作为核心检测模型，该算法是一种基于随机森林\cite{Breiman01}的无监督异常检测算法，通过构建多棵孤立树来识别异常点。异常点由于其特殊性，在随机划分过程中更容易被孤立，因此路径长度更短。
...
本实验选择Isolation Forest算法\cite{Liu08}作为异常检测的核心模型，主要基于以下考虑：
...
Isolation Forest算法\cite{Liu08}的核心思想是通过随机划分数据空间，将异常点与正常点分离，这一思想来源于集成学习理论\cite{Zhou18}。异常点由于其特殊性，在随机划分过程中更容易被孤立，因此路径长度更短。
...
\begin{figure}[ht]
 \centering
 \includegraphics[height=8cm]{confusion_matrix.png}
 \caption{异常检测混淆矩阵热力图}
 \label{fig:confusion}
\end{figure}
...
\begin{figure}[ht]
 \centering
 \includegraphics[height=8cm]{anomaly_scores.png}
 \caption{异常分数分布}
 \label{fig:scores}
\end{figure}
...
\begin{figure}[ht]
 \centering
 \includegraphics[height=8cm]{feature_importance.png}
 \caption{特征重要性分析}
...
本实验设计并实现了一个基于Isolation Forest算法\cite{Liu08}的物联网数据异常检测系统，该算法属于集成学习算法\cite{Zhou18}，通过模拟生成的物联网传感器数据集验证了算法的有效性。
